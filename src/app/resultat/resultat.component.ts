...
export class ResultatComponent implements OnInit, OnDestroy {
  ...
  constructor(private router: Router,private tService : TestCovidService ,private vService : VaccinService ,private cvService : CarteVaccinService  ,private storage: Storage,private barcodeScanner: BarcodeScanner, private hService: HistoriqueService) {}
    ...
    if(chemin == "vaccin"){
      let carte: CarteVaccination = await this.storage.get('json.carte');
        if (carte) {
          ...
          this.cvService.getCarteByPersonne(this.personne._id).subscribe(async (data: any) => {
  ...
  scanTest() {
    this.barcodeScanner.scan().then(async barcodeData => {
      ...
      this.tService.getTest(testID).subscribe((data: any) => {
        var test: Test = data;
        if(!test.personne_id || test.personne_id == null || test.personne_id == undefined){
  ...
  scanVaccin() {
    this.barcodeScanner.scan().then(async barcodeData => {
      ...
      this.vService.getVaccin(vaccinID).subscribe((data: any) => {
        ...
        if(!vaccin.carte_id || vaccin.carte_id == null || vaccin.carte_id == undefined){
          alert("ceci n'est pas un de vos test");
        }
        else{
